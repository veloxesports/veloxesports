...
if (!connectionString) throw new Error("DATABASE_URL must be configured before seeding Khemora.");

const pool = new Pool({ connectionString });
const prisma = new PrismaClient({ adapter: new PrismaPg(pool) });

async function main() {
  console.log("Seeding Khemora development data...");
  ...
  for (const seedUser of developmentUsers) {
    const user = await prisma.user.upsert({
      ...
      create: {
        ...
        role: seedUser.role,
        profile: { create: { khemoraUsername: seedUser.username, rank: seedUser.rank, xp: seedUser.xp, level: seedUser.level, wins: seedUser.wins, losses: seedUser.losses, tournamentWins: seedUser.tournamentWins, favoriteGames: ["FC26", "Chess"] } },
        wallet: { create: {} },
      },
  ...
  const tournamentData = [
    ...
    { title: "Khemora Chess Masters", slug: "khemora-chess-masters", gameId: chess.id, prizePool: 500, entryFee: 0, isPaid: false, maxParticipants: 64, registrationDeadline: new Date(Date.now() + 5 * 86_400_000), startDate: new Date(Date.now() + 6 * 86_400_000), format: TournamentFormat.SWISS, status: TournamentStatus.REGISTRATION_OPEN, region: "Global", gameMode: "Rapid", rules: "Fair-play rules apply. Use the published match link and report issues immediately.", prizes: [{ placement: 1, amount: 300 }, { placement: 2, amount: 150 }, { placement: 3, amount: 50 }] },
  ];
  ...
  const achievements = [
    { name: "First Victory", description: "Win your first confirmed Khemora match.", iconUrl: "⚔️", xpReward: 100, criteria: { type: "MATCH_WINS", target: 1 } },
    ...
    { name: "Tournament Champion", description: "Win a Khemora tournament.", iconUrl: "🏆", xpReward: 750, criteria: { type: "TOURNAMENT_WINS", target: 1 } },
  ];
  ...
  await prisma.notification.upsert({
    ...
    create: { id: "00000000-0000-4000-8000-000000000001", userId: users[0].id, type: "SYSTEM", title: "Khemora development data ready", message: "Development seed data is ready. No production-like payment confirmations were created." },
  });
  ...
  console.log("Khemora development data seeded without payment events.");
}
