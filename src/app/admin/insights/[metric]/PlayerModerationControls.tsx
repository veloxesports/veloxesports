  ...
  return (
    <div className="mt-4 border-t border-[#29342a] pt-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-[10px] font-black uppercase tracking-[0.1em] text-[#aeb8ad]">Super Admin controls</p>
        <p className="text-[11px] text-[#718071]">Changes are permanently audit logged.</p>
      </div>
        ...
        {actions.map((action) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center rounded-xl border px-3 py-2 text-xs font-black transition disabled:cursor-not-allowed disabled:opacity-50 ${detail.className}`}
            >
              <Icon className="mr-1.5 h-3.5 w-3.5" aria-hidden />
              {pendingAction === action ? "Updating…" : detail.label}
            </button>
          );
        })}
      </div>
      {message && (
        <p role="status" className="mt-3 flex items-start gap-2 text-xs leading-relaxed text-[#d8e6d2]">
          <CircleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#c5f94d]" aria-hidden />
          {message.replace("this player", playerName)}
        </p>
      )}
      ...
      {confirmAction && (
        <AdminConfirmModal
          ...
          title={`Confirm ${details[confirmAction].label}`}
          message={details[confirmAction].confirmation.replace("this player", `“${playerName}”`)}
          confirmLabel={details[confirmAction].label}
          variant={details[confirmAction].isDanger ? "danger" : "default"}
