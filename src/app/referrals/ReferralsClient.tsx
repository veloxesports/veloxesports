...
export function ReferralsClient({ initialData }: { initialData: ReferralData }) {
  ...
  async function redeem(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(false);
    setMessage(result.success ? "Referral code applied. Welcome to Khemora!" : result.error ?? "We couldn't redeem that code.");
  ...
  return (
    <main className="velox-page text-slate-100">
      ...
      <section className="mt-7 rounded-[26px] border border-[#49633d] bg-gradient-to-br from-[#20351a] to-[#111811] p-5"><Gift className="h-7 w-7 text-[#c5f94d]" aria-hidden /><p className="mt-5 text-[11px] font-black uppercase tracking-[0.13em] text-[#aeb8ad]">Your Khemora referral code</p><div className="mt-2 flex items-center justify-between gap-3 rounded-2xl border border-[#3a4d38] bg-[#090d09]/70 px-4 py-3"><code className="text-xl font-black tracking-[0.18em] text-white">{initialData.code}</code><Button onClick={copyCode} variant="outline" className="h-9 border-[#49633d] bg-[#182319] text-[#c5f94d] hover:bg-[#22311e] transition active:scale-95">{copied ? (<span className="flex items-center gap-1 text-xs font-black"><Check className="h-4 w-4 text-[#c5f94d]" aria-hidden />Copied!</span>) : (<><Copy className="h-4 w-4" aria-hidden /><span className="sr-only">Copy code</span></>)}</Button></div><p className="mt-4 text-xs leading-relaxed text-[#aeb8ad]">Referral promotions, if enabled by Khemora, are recorded server-side. A referral code never transfers Telegram Stars on its own.</p></section>
      ...
      <section className="mt-8"><h2 className="flex items-center gap-2 text-lg font-black uppercase tracking-[0.05em] text-white"><Users className="h-5 w-5 text-[#c5f94d]" aria-hidden />Your referrals</h2>{initialData.referrals.length === 0 ? <p className="velox-card mt-4 p-5 text-sm text-[#8e998f]">No completed referrals yet.</p> : <div className="mt-4 space-y-2">{initialData.referrals.map((referral) => <div key={referral.id} className="velox-card flex items-center justify-between p-4"><div><p className="font-bold text-white">{referral.referredUser?.username ?? referral.referredUser?.firstName ?? "Khemora player"}</p><p className="mt-1 text-xs text-[#8e998f]">{referral.completedAt ? new Date(referral.completedAt).toLocaleDateString() : "Pending"}</p></div><span className="flex items-center gap-1 text-xs font-black text-[#c5f94d]"><Check className="h-4 w-4" aria-hidden />{referral.status}</span></div>)}</div>}</section>
    </main>
  );
}
