...
export function NotificationSheet({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 backdrop-blur-md transition-opacity duration-200 animate-in fade-in"
      onClick={onClose}
    >
      <div
        className="flex max-h-[88dvh] w-full max-w-xl flex-col rounded-t-[32px] border-t border-[#314232] bg-[#0c130d] pb-[max(20px,env(safe-area-inset-bottom))] shadow-[0_-20px_60px_rgba(0,0,0,0.85)] animate-in slide-in-from-bottom-8 duration-300"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Grab bar */}
        <div className="mx-auto mt-3 h-1.5 w-12 rounded-full bg-[#29382b]" aria-hidden />

        {/* Header */}
        <div className="flex items-center justify-between border-b border-[#202d21] px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-black tracking-tight text-white">Alerts & Updates</h2>
                {unreadCount > 0 && (
              ...
              <p className="text-[11px] font-semibold text-[#809081]">Tournament, match, and wallet events</p>
            </div>
          </div>
        ...
        {/* Filter Pills */}
        <div className="flex gap-1.5 overflow-x-auto px-5 py-2.5 scrollbar-hide">
          {filterTabs.map((tab) => {
